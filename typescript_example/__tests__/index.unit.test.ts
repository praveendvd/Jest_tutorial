...
describe('Validate Index.js', () => {
    ...
    it('Should call catch if inquirer throws some error', async () => {
        let inquirerMock = jest.mock('inquirer', () => {
            return {
                ...jest.requireActual('inquirer') as Inquirer,
                prompt: jest.fn().mockImplementationOnce(async function () { throw new Error('Catch error') })
            }
        });
        restArgs({ n: "yourname", i: true });
        require('../index')
        await expect(await (jest.requireMock('inquirer') as Inquirer).prompt).toHaveBeenCalled();
    ...
    it('Should call inquirer if -i is provided', async () => {
        ...
        restArgs({ n: "yourname", i: true });
        require('../index')
        expect((jest.requireMock('inquirer') as Inquirer).prompt).toHaveBeenCalled();
        expect(process.exit).not.toHaveBeenCalled();
        expect(console.log).not.toHaveBeenCalled();
    })
...
function restArgs({ n , i }: { n?: string, i?: boolean|string }): void {
    while (process.argv.length > 2) {
        process.argv.pop();
    }
    if (n) process.argv.push('-n', n);
    if (i) process.argv.push('-i', i as string);
}
